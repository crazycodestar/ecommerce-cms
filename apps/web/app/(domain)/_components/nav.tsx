          ...
          <div className="flex gap-2">
            <Sheet>
              ...
              <SheetContent side="left" className="w-full md:hidden">
                ...
                <div>
                  {categories?.slice(0, 8).map(({ name, slug, _id }, index) => (
        ...
        <nav className="hidden md:flex items-center justify-around text-sm py-5 px-8">
          ...
          {categories?.slice(0, 8).map(({ name, slug, _id }, index) => (
            <div
              key={index}
              className="relative"
              onMouseEnter={() => setActiveId(slug)}
              onMouseLeave={() => setActiveId(null)}
            >
              <Link
                href={`/${slug}`}
                className="px-3 py-1 whitespace-nowrap font-medium hover:underline"
              >
                {name}
              </Link>
              {/* Subcategory Menu */}
              {activeId === slug && (
                <div className="absolute top-full left-0 bg-white shadow-lg rounded-md py-2 min-w-[200px] z-50">
                  <SubCategoryMenu parentId={_id} storeSlug={storeSlug} />
                </div>
              )}
            </div>
          ))}
...
const SubCategoryMenu = ({
  ...
  return (
    <div className="flex flex-col gap-2 px-1">
      {subCategories?.map(({ name, slug }, index) => (
        <Link
          key={index}
          href={`/${slug}`}
